...
Now, both `commat` and `z` contain `r nrow(commat)` rows for each plot and `r ncol(commat)` columns for each species for which true occurrences were estimated.
...
mod <- lm(qlogis(P) ~ sla + ch + sm + nocc_sd, data = d)
...
mod1 <- lm(qlogis(P) ~ sla + ch + sm + nocc_sd, data = d[d$nobs >= 23, ])
round(summary(mod1)$coef, 3)
```

Functional ecology is often conducted along environmental gradient. We therefore asked whether the strength of detection filtering is changing along the elevational gradient. For each community we thus calculate the average species' detection probability of the species truely occurring in the communities and plot average detection probability of a community along the elevational gradient (Fig 1a). This pattern 

```{r}
# Community mean of species' detection probabilities
CM_P_cor <- apply(z, 1, function(x) mean(P[x==1], na.rm = TRUE))
CM_P_obs <- apply(commat, 1, function(x) mean(P[x==1], na.rm = TRUE))

# Community mean of species' total occurrence
Nocc_cor <- apply(z, 2, sum)
Nocc_obs <- apply(z, 2, sum)
Av_Spec_occ <- apply(z, 1, function(x) mean(Nocc[x==1], na.rm = TRUE))
```


```{r fig1, cache=TRUE, fig.width = 7, fig.height = 3.5, echo=FALSE}
# (a) Plot average species' detection probability of communities along elevation
par(mfrow = c(1, 2), mar = c(2.5, 4, 1, 1), oma = c(1, 1, 1, 1))
elrange <- 250:2750
nsim <- 10
d <- data.frame(cbind(ele = plantsBDM$elevation, dep = CM_P))
plot(d$ele, d$dep, axes = FALSE, pch = 21, cex = 0.5, col = "grey50", bg = "grey80",
...
mtext(text = "(a) Community average of speices' detection probability", side = 3, at = -420, line = 1, cex = 0.7, adj = 0)
...
# (b) Plot average species' number of occurences of communities along elevation
d <- data.frame(cbind(ele = plantsBDM$elevation, dep = Av_Spec_occ))
plot(d$ele, d$dep, axes = FALSE, pch = 21, cex = 0.4,  bg = "grey80",
     col = "grey50", ylim = c(100, 200), xlim = c(250, 2750),  ylab = "", xlab = "")
axis(side=1, at = seq(250, 2750, 250), labels = rep("", 11), pos=100)
mtext(seq(500, 2500, 500), 1, at = seq(500, 2500, 500), cex = 0.7)
axis(side = 2, at = seq(100, 200, 20), labels = rep ("", 6), pos = 250)
mtext(seq(100, 200, 20), 2, at = seq(100, 200, 20), cex = 0.7, las = 1)
mtext(text = "Elevation", side = 1, line = 1, cex = 0.7)
mtext(text = "Average species' total\noccurrence", side = 2, line = 1, cex = 0.7)
mtext(text = "(b) Community average of speices' total occurrences", side = 3, at = -420, line = 1, cex = 0.7, adj = 0)
pred <- predict(gam(dep ~ s(ele), data = d), newdata = data.frame(ele=elrange), type = "response")
points(elrange, pred, ty = "l", lty = 1, lwd = 2)
simres <- array(NA, dim = c(length(elrange), nsim))
for(s in 1:nsim) {
...
**Fig. 1:** Elevational pattern of (a) average species' detection probability and (b) average species' total occurrence (i.e. the number of site a species is occurring) of species in the studied communities. The solid lines represent the predictions and the dashed line the bootstrapped 95% confidence intervals from the generalized additive model (GAM).
