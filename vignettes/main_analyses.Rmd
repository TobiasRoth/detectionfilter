---
title: "Analyses of plant data from the Swiss Biodiversity Monitoring (BDM)"
...
# Introduction
## Prerequisite
This vignette presents analyses from Roth, Allan, Pearman and Amrhein "Functional ecology and imperfect detection of species" (in press in the journal Methods in Ecology and Evolution). The latest version of the package `detecionfilter` is required to run the code and should be downloaded from github (make sure you have the package `devtools` installed).
...
We call the assemblages of species that occur at a plot a _community_. In the case of `plantsBDM` we speak of `meta-community` data because the data set contains observations of 362 communities. Further, note that the array `y[i,k,j]` contains information on whether or not a species was _observed_. When we refer to species _occurrence_, we mean the true occurrence state of the species, which is not directly observable during a survey because of imperfect detection of species.
...
```{r, message=FALSE, warning=FALSE, cache=TRUE}
# Merge observations from two visits
...
For all further analyses, trait values were log transformed (Westoby 1998), then normalized to a mean of 0 and standard deviation of 1, allowing comparison among traits (Schielzeth 2010).
...
We analysed species with at least four detections because otherwise the algorithm implementing the single season occupancy model failed to converge consistently. We thus removed `r sum(apply(commat_obs, 2, sum) <= 3)` species that are `r round(100 * sum(apply(commat_obs, 2, sum) <= 3) / nspec, 1)`% of all recorded species.
...
Finally, we bundle the results into two objects: a vector `P` containing the average detection probability per species, and the detection-corrected meta-community matrix `z[i,k]` containing the (estimated) true occurrence of species `k` at site `ì`.
...
mod <- lm(qlogis(P) ~ sla + I(sla^2) + ch + I(ch^2) + sm + I(sm^2) + nocc_sd + I(nocc_sd^2) + meanel_sd + I(meanel_sd^2), data = d)
round(summary(mod)$coef, 3)


tt <- residuals(mod)
m <- lm(qlogis(P) ~ sla + ch + sm + nocc_sd + meanel_sd, data = d[names(tt[tt<10]), ])
round(summary(m)$coef, 3)
plot(m)

par(mar = c(5,5,1,3), mfrow = c(2,3))
visreg(m, "sla", line=list(col="red"), points=list(cex=0.5, pch=16), 
       xlab = "Specific leaf area (SLA)", ylab = "Species detection probability\n(logit-transformed)")
visreg(m, "ch", line=list(col="red"), points=list(cex=0.5, pch=16), 
       xlab = "Canopy height (CH)", ylab = "Species detection probability\n(logit-transformed)")
visreg(m, "sm", line=list(col="red"), points=list(cex=0.5, pch=16), 
       xlab = "Seed mass (SM)", ylab = "Species detection probability\n(logit-transformed)")
visreg(m, "nocc_sd", line=list(col="red"), points=list(cex=0.5, pch=16), 
       xlab = "Number of occurrences", ylab = "Species detection probability\n(logit-transformed)")
visreg(m, "meanel_sd", line=list(col="red"), points=list(cex=0.5, pch=16), 
       xlab = "Average elevation of occurrence", ylab = "Species detection probability\n(logit-transformed)")
...
The number of days between the first and second visit may indicate the proportion of the vegetation period sampled in the surveys. We asked whether the number of days between first and second visit changes with elevation.
...
Let us look at a few of the species that are responsible for these patterns.
...
In communities below 750m, the species that most often remained undetected were *Buglossoides arvensis* (species-ID: 1575), a weed of arable land and *Helianthus tuberosus* (species ID 2094), a currently spreading invasive species with late flowering. The species that most often remained undetected in communities between 750m and 1250m was *Descurainia sophia* (species ID 613), a ruderal of fields and dry rock faces.


# Community composition and diversity along the elevational gradient
...
We also calculated for each community whether the effect of detection filtering is relevant. We define an effect of detection filtering on community composition as being relevant if it is larger than the change of community composition we observe per 100m along the elevational gradient. 
...
```{r fig2.1, cache=TRUE, fig.width = 10, fig.height = 3.5, echo = FALSE}
# Plot settings 
par(mfrow = c(1, 3), mar = c(2.5,4,3,1.5), oma = c(1,1,1,1)) 
...
Adding confidence intervals to Fig. 2.1 resulted in overloaded figures. We thus re-drew the figures with removing the CMs for the 362 communities and adding the bootstrapped 95% confidence intervals for the predictions from the generalized additive model. 

```{r fig2.2, cache=TRUE, fig.width = 10, fig.height = 3.5, echo = FALSE}
# Plot settings
par(mfrow = c(1, 3), mar = c(2.5,4,3,1.5), oma = c(1,1,1,1)) 
...
## Community diversity along the elevational gradient
...
```{r fig3.2, cache=TRUE, fig.width = 10, fig.height = 3.5, echo = FALSE}
# Plot settings 
par(mfrow = c(1, 3), mar = c(2.5,4,3,1.5), oma = c(1,1,1,1))
...
```{r fig3.3, cache = TRUE, fig.width = 10, fig.height = 3.5, echo = FALSE}
# Plot settings 
par(mfrow = c(1, 3), mar = c(2.5,4,3,1.5), oma = c(1,1,1,1))
