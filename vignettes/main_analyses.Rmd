...
```{r fig1, cache=TRUE, fig.width = 5, fig.height = 3.5}
# Community mean of species' detection probabilities
CM_P <- apply(z, 1, function(x) mean(P[x==1], na.rm = TRUE))

# Make plot
par(mfrow = c(1, 1), mar = c(2.5, 4, 1, 1))
plot(plantsBDM$elevation, CM_P, pch = 16, cex = 0.4, axes = FALSE, 
     ylim = c(0.5, 0.8), xlim = c(250, 2750),  ylab = "", xlab = "")
axis(side=1, at = seq(250, 2750, 250), labels = rep("", 11), pos=0.5)
mtext(seq(500, 2500, 500), 1, at = seq(500, 2500, 500), cex = 0.7)
axis(side = 2, at = seq(0.5, 0.8, 0.05), labels = rep ("", 7), pos = 250)
mtext(seq(0.5, 0.8, 0.1), 2, at = seq(0.5, 0.8, 0.1), cex = 0.7, las = 1)
mtext(text = "Elevation", side = 1, line = 1, cex = 0.7)
mtext(text = "Average species' detection\nprobability", side = 2, line = 1, cex = 0.7)
```

**Fig. 1:** Elevational pattern of average species' detection probability of species occuring in a community.  
